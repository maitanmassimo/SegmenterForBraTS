...
def eval_dataset(
    ...
    filtered_total_seg_pred_maps = {}
    filtered_total_seg_gt_maps = {}
    print(len(seg_gt_maps)/128)
    for n in range(int((len(seg_gt_maps)+3)/128)):
        
        print("patient n: {}".format(n))
        print("{} -> {}".format(n*128+2, ((n+1)*128-1)))
        seg_pred_maps_patient = dict(itertools.islice(seg_pred_maps.items(), n*128,((n+1)*128-2)))
        seg_gt_maps_patient = dict(itertools.islice(seg_gt_maps.items(), n*128,((n+1)*128-2)))
    ...
    if ptu.dist_rank == 0:
        ...
        with open(model_dir / f"test_scores_tc_{suffix}_per patient.yml", "w") as f:
    ...
    if ptu.dist_rank == 0:
        ...
        with open(model_dir / f"test_scores_{suffix}_tc.yml", "w") as f:
            f.write(scores_str)
